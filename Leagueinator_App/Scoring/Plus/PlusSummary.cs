    ...
    public class PlusSummary : DataTable {
        ...
        public bool HasRow(int team) {
            var rows = this.AsEnumerable()
                           .Where(row => row.Field<int>(MatchTable.COL.TEAM_IDX) == team)
                           .ToList();
        ...
        public DataRow GetRow(int team) {
            ...
            if (rows.Count == 0) throw new KeyNotFoundException($"{MatchTable.COL.TEAM_IDX} = {team}");
            return rows[0];
        }
        ...
        private DataRow AddOrGet(int team) {
            if (!this.HasRow(team)) {
                var row = this.NewRow();
                row[MatchTable.COL.TEAM_IDX] = team;
                this.Rows.Add(row);
                return row;
            }
        ...
        private void Build() {
            ...
            this.Columns.Remove(MatchTable.COL.ROUND);
            this.Columns.Remove(MatchTable.COL.LANE);
            this.Columns.Remove(MatchTable.COL.UID);
            this.Columns.Remove(MatchTable.COL.TIE);
        }

        private void Fill() {
            foreach (DataRow roundRow in this.PlusRounds.Rows) {
                int teamIndex = roundRow.Field<int>(MatchTable.COL.TEAM_IDX);
